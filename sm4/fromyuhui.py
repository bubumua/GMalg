...
def T(x):  # x:32bits,
    ...
    for i in range(0, 8, 2):  # 分为四组
        B += int2bit(SboxTable[int(x[i * 4:(i + 1) * 4], 2) * 16 + int(x[(i + 1) * 4:(i + 2) * 4], 2)],
                     8)  # 高四位为排，低四位为列。
...
def f(x):  # Rk:32bits
    ...
    for j in range(32):
        t = xor(x[j * 32:(j + 1) * 32], T(xor(x[(j + 1) * 32:(j + 2) * 32], xor(x[(j + 2) * 32:(j + 3) * 32],
                                                                                xor(Roundk[j], x[(j + 3) * 32:(
                                                                                                                      j + 4) * 32])))))  # x128bits，分为四组进行异或操作。
        x += t
    return x[35 * 32:36 * 32] + x[34 * 32:35 * 32] + x[33 * 32:34 * 32] + x[32 * 32:33 * 32]
